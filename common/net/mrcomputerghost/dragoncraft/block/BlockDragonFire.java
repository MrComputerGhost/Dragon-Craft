...
public class BlockDragonFire extends BlockFire {
    private int[] chanceToEncourageFire = new int[4096];
    private int[] abilityToCatchFire = new int[4096];
    ...
    public void initializeBlock() {
        ...
        setBurnRate(Block.blockGold.blockID, 5, 20);
    ...
    public void onBlockAdded(World par1World, int par2, int par3, int par4) {
        ...
        if ((par1World.getBlockId(par2, par3 - 1, par4) != Block.blockGold.blockID) || (!Blocks.dragonPortal.get().tryToCreatePortal(par1World, par2, par3, par4))) {
            if ((!par1World.doesBlockHaveSolidTopSurface(par2, par3 - 1, par4)) && (!canNeighborBurn(par1World, par2, par3, par4))) {
                par1World.setBlockToAir(par2, par3, par4);
            } else {
    ...
    public void randomDisplayTick(World par1World, int par2, int par3, int par4, Random par5Random) {
        ...
        if ((!par1World.doesBlockHaveSolidTopSurface(par2, par3 - 1, par4)) && (!BlockDragonPortal.canBlockCatchFire(par1World, par2, par3 - 1, par4, ForgeDirection.UP))) {
            if (BlockDragonPortal.canBlockCatchFire(par1World, par2 - 1, par3, par4, ForgeDirection.EAST)) {
                for (int l = 0; l < 2; l++) {
            ...
            if (BlockDragonPortal.canBlockCatchFire(par1World, par2 + 1, par3, par4, ForgeDirection.WEST)) {
                for (int l = 0; l < 2; l++) {
            ...
            if (BlockDragonPortal.canBlockCatchFire(par1World, par2, par3, par4 - 1, ForgeDirection.SOUTH)) {
                for (int l = 0; l < 2; l++) {
            ...
            if (BlockDragonPortal.canBlockCatchFire(par1World, par2, par3, par4 + 1, ForgeDirection.NORTH)) {
                for (int l = 0; l < 2; l++) {
                    float f = par2 + par5Random.nextFloat();
                    float f1 = par3 + par5Random.nextFloat();
            ...
            if (BlockDragonPortal.canBlockCatchFire(par1World, par2, par3 + 1, par4, ForgeDirection.DOWN)) {
                for (int l = 0; l < 2; l++) {
                    float f = par2 + par5Random.nextFloat();
	...
	{
		this.blockIcon = reg.registerIcon("DC:RadioFire"); 
	}
